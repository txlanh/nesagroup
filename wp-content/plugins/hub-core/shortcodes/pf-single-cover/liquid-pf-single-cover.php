<?php
/**
* Shortcode Porftfolio Single Cover
*/
...
* LD_Shortcode
*/
class LD_Pf_Single_Cover extends LD_Shortcode {
	...
	public function __construct() {
		...
		// Properties
		$this->slug = 'ld_single_portfolio_cover';
		$this->title = esc_html__( 'Portfolio Single Cover', 'landinghub-core' );
		$this->icon = 'la la-folder';
		$this->description = esc_html__( 'Display Portfolio Single Post Cover', 'landinghub-core' );
	...
	protected function generate_css() {
		...
		*/
		$elements[ liquid_implode( '%1$s .lqd-pf-single-cover .lqd-pf-single-title' ) ] = array( $text_font_inline_style );
		$elements[ liquid_implode( '%1$s .lqd-pf-single-cover .lqd-pf-single-title' ) ]['font-size'] = !empty( $fs ) ? $fs : '';
		$elements[ liquid_implode( '%1$s .lqd-pf-single-cover .lqd-pf-single-title' ) ]['line-height'] = !empty( $lh ) ? $lh : '';
		$elements[ liquid_implode( '%1$s .lqd-pf-single-cover .lqd-pf-single-title' ) ]['font-weight'] = !empty( $fw ) ? $fw . ' !important' : '';
		$elements[ liquid_implode( '%1$s .lqd-pf-single-cover .lqd-pf-single-title' ) ]['letter-spacing'] = !empty( $ls ) ? $ls : '';

		if( ! empty( $heading_color ) && isset( $heading_color ) ) {
			$elements[ liquid_implode( '%1$s .lqd-pf-single-cover .lqd-pf-single-title' ) ]['color'] = $heading_color;
		}
