...
class LD_Shortcode extends WPBakeryShortCode {
	...
	public function __construct() {
		...
		if( ! isset( $this->slug ) && empty( $this->slug ) ) {
			wp_die( esc_html__( 'Please define slug', 'landinghub-core' ), esc_html__( 'Variable Missing', 'landinghub-core' ) );
		}
	...
	protected function set_config() {
		...
		$shortcode = array(
			...
			'name' => $this->title,
			'params' => $this->params,
			'category' => ! empty( $this->category ) ? $this->category : esc_html__( 'LiquidThemes', 'landinghub-core' )
		);
	...
	public function prepare_params() {
		...
		foreach( $this->params as $id => &$param ) {
			...
			if( empty( $param['description'] ) && isset( $param['heading'] ) && 'subheading' != $param['type'] ) {
				$prefix = esc_html__( 'Select ', 'landinghub-core' );

				if( in_array( $param['type'], array( 'textarea', 'textfield', 'textarea_html' ) ) ) {
					$prefix = esc_html__( 'Enter  ', 'landinghub-core' );
				}
	...
	public function render( $atts, $content = '' ) {
		...
		if ( !$located ) {
			trigger_error( sprintf( esc_html__( 'Template file is missing for `%s` shortcode. Make sure you have `%s` file in your theme folder or default folder.', 'landinghub-core' ), $this->title, 'view.php' ) );
			return;
		}
	...
	protected function google_fonts_style( $fontsData ) {
		...
		if (isset($fontStyles[1])) $styles['font-weight'] = $fontStyles[1] . '!important';
		if (isset($fontStyles[2])) $styles['font-style'] = $fontStyles[2] . '!important';
	...
	public function get_wordpress_data( $type = false, $args = array() ) {
		...
		$data = array(
			esc_html__( 'Select option', 'landinghub-core' ) => ''
		);
