<?php
 if ( $element_name === 'ld_media_element' ) {
    ...
    $widget_utils['lqdsep-utils-flex-justify-content-center'] = array(
        'conditions' =>
            $this->get_repeater_item_condition($element, 'items', 'vertical_alignment', 'justify-content-center') ||
            (
                $this->get_repeater_item_condition($element, 'items', 'add_icon', 'yes') &&
                (
                    $this->get_repeater_item_condition($element, 'items', 'icon_type', 'video') ||
                    $this->get_repeater_item_condition($element, 'items', 'icon_type', 'video2')
                )
            )
    );
    $widget_utils['lqdsep-utils-flex-justify-content-end'] = array(
        'conditions' =>
            $this->get_repeater_item_condition($element, 'items', 'vertical_alignment', 'justify-content-end')
    );
