...
class WPLPInstall
{
    ...
    public function wplpUpdateVersion()
    {
        ...
        //add content selection for all theme when update version to 5.0.2
        if (version_compare($option_ver, '5.0.2', '<')) {
            $meta_key = '_wplp_settings';
            $postsId  = $wpdb->get_results($wpdb->prepare('SELECT post_id FROM '.$wpdb->postmeta.' WHERE meta_key = %s ', $meta_key));
            ...
            if (!empty($postsId)) {
                foreach ($postsId as $post) {
                    $settings = get_post_meta($post->post_id, $meta_key, true);
                    if ($settings['theme'] === 'masonry-category' || $settings['theme'] === 'portfolio') {
                        $settings['dfThumbnail'] = 'Thumbnail';
                        $settings['dfTitle'] = 'Title';
                        $settings['dfCategory'] = 'Category';
                        $settings['dfAuthor'] = '';
                        $settings['dfDate'] = '';
                        $settings['dfText'] = '';
                        $settings['dfReadMore'] = '';
                    } elseif ($settings['theme'] === 'smooth-effect'
                        || $settings['theme'] === 'timeline'
                        || $settings['theme'] === 'material-vertical'
                        || $settings['theme'] === 'material-horizontal'
                    ) {
                        $settings['dfThumbnail'] = 'Thumbnail';
                        $settings['dfTitle'] = 'Title';
                        $settings['dfCategory'] = 'Category';
                        $settings['dfAuthor'] = '';
                        $settings['dfDate'] = 'Date';
                        $settings['dfText'] = 'Text';
                        $settings['dfReadMore'] = 'dfReadMore';
                    } elseif ($settings['theme'] === 'masonry') {
                        $settings['dfThumbnail'] = 'Thumbnail';
                        $settings['dfTitle'] = 'Title';
                        $settings['dfCategory'] = '';
                        $settings['dfAuthor'] = '';
            ...
            update_option($ver, '5.0.2');
        }

        if (version_compare($option_ver, '5.0.3', '<')) {
            $meta_key = '_wplp_settings';
            $postsId  = $wpdb->get_results($wpdb->prepare('SELECT post_id FROM '.$wpdb->postmeta.' WHERE meta_key = %s ', $meta_key));
            ...
            if (!empty($postsId)) {
                foreach ($postsId as $post) {
                    $settings = get_post_meta($post->post_id, $meta_key, true);
                    $settings['addLink'] = 'yes';
                    $settings['linkThumbnail'] = 'yes';
                    $settings['linkTitle'] = 'yes';
                    $settings['linkText'] = '';
                    $settings['linkDate'] = '';
                    $settings['linkCategory'] = '';
                    $settings['linkAuthor'] = '';
                    $settings['linkReadMore'] = 'yes';

                    update_post_meta($post->post_id, '_wplp_settings', $settings);
                }
            }
            update_option($ver, '5.0.3');
        }
    }
}
