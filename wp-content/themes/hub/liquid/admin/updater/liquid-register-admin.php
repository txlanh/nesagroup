...
class Liquid_Register {
	...
	function admin_error() {
		echo '<div class="error"><p>' . sprintf( wp_kses_post( __( 'The %s theme needs to be registered. %sRegister Now%s', 'hub' ) ), 'Hub', '<a href="' . admin_url( 'admin.php?page=liquid') . '">' , '</a>' ) . '</p></div>';
	}
	
	function admin_notice() {
		echo '<div class="notice"><p>' .
                         sprintf( wp_kses_post( __( 'Purchase code is invalid. Need a license? %sPurchase Now%s', 'hub' ) ), '<a target="_blank" href="https://themeforest.net/item/hub-responsive-multipurpose-wordpress-theme/31569152">', '</a>' ) .
            '</p></div>';
	}
	...
	function messages() {
		...
		if ( ! $license ) {
			$message    = '<div class="lqd-dsd-box-head-inner">
								<h4>' . esc_html__( 'Register Hub', 'hub' ) . '</h4>
	...
	function form() {
		...
		<form action="options.php" method="post" class="lqd-dsd-register-form">
			...
			<input id="hub_purchase_code" name="hub_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'hub' ); ?>">
			<input id="hub_register_email" name="hub_register_email" type="email" value="<?php echo esc_attr( $email ); ?>" placeholder="<?php esc_attr_e( 'Enter your email', 'hub' ); ?>">
			<input type="submit" value="<?php esc_attr_e( 'Register your copy', 'hub' ) ?>">
		</form>
		<?php
	}
