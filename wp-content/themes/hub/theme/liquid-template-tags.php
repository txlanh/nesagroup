...
function liquid_portfolio_media( $args = array() ) {
	...
	elseif( 'gallery' === $format && $gallery = liquid_helper()->get_option( 'post-gallery-ids' ) ) {
		...
		echo '<div class="carousel-container carousel-nav-floated carousel-nav-center carousel-nav-middle carousel-nav-xl carousel-nav-solid carousel-nav-rectangle">';
			...
			echo '<div class="carousel-items row mx-0" data-lqd-flickity=\'{ "prevNextButtons": true, "navArrow": "1", "pageDots": false, "navOffsets":{"prev":"28px","next":"28px"}, "parallax": true, "adaptiveHeight": true }\'>';
				...
				foreach ( $gallery as $item ) {
					...
					printf( '<div class="carousel-item col-xs-12 px-0"><figure class="pos-rel"><img src="%s" alt="%s">%s</figure></div>',$resized_image , esc_attr( $item['title'] ), $lightbox === 'on' ? liquid_get_lightbox_link( $resized_image, 'lqd-post-gallery' ) : '' );
...
function liquid_get_lightbox_link( $link_to_image ) {
	...
	return '<a class="lightbox-link fresco pos-abs w-100 h-100 pos-tl" data-type="image" data-fresco-group="' . $group_id . '" href="' . esc_url( $link_to_image ) . '"></a>';
}
