...
class Liquid_Theme_Assets extends Liquid_Base
{
	...
	public function register()
	{
		...
		$this->style('font-awesome-v5', $this->get_vendor_uri('font-awesome-v5/css/all.min.css'));
		...
		if (
			...
			$deps = array(
				...
				'scrollTrigger',
				'liquid-draw-shape',
				'liquid-animated-blob',
				'jquery-fontfaceobserver',
				'intersection-observer',
			);
		} else {
			if ( liquid_helper()->get_theme_option('combine_js') == 'on' && !empty( liquid_helper()->get_theme_option('google-api-key') ) ) {
				$this->script('google-maps-api', $this->google_map_api_url());
			}
			$deps = array();
		}
		...
		if (!is_array(get_option('liquid_scrips'))) {
			update_option('liquid_scrips', array(
				...
				'gsap-draw-shape-plugin' => 'GSAP draw shape plugin',
				'jquery-vivus' => 'Vivus',
				'flickity' => 'Flickity',
				...
				'liquid-draw-shape' => 'Liquid draw shape',
				'liquid-animated-blob' => 'Liquid animated blob',
			));
		}
	}
	...
	public function enqueue()
	{
		...
		if ( liquid_helper()->get_theme_option('load_fontawesome_v5') === 'on' ) {
		...
		if (
			class_exists('Liquid_Elementor_Addons') && (liquid_helper()->is_page_elementor() || !liquid_helper()->is_page_elementor()) ||
			(defined('ELEMENTOR_VERSION') && \Elementor\Plugin::$instance->preview->is_preview_mode())
		) {
