...
$post_author_meta_enable = $post_author_meta_enable ? $post_author_meta_enable : liquid_helper()->get_option( 'post-single-meta-enable' );
...
<div class="entry-meta d-flex flex-wrap align-items-center text-center">
	...
	<?php if ( liquid_helper()->get_option( 'blog-post-modified-date' ) === 'yes' && get_the_date() != get_the_modified_date() ){ ?>
		<div class="post-last-updated posted-on">
			<span><?php esc_html_e( 'Last updated:', 'hub' ); ?></span>
			<a href="<?php the_permalink(); ?>" rel="bookmark">
				<?php 
				
					$time_string = '<time class="entry-date last-updated" datetime="%1$s">%2$s</time>';
					printf( $time_string,
