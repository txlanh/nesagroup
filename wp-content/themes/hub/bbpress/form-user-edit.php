...
<form id="bbp-your-profile" action="<?php bbp_user_profile_edit_url( bbp_get_displayed_user_id() ); ?>" method="post" enctype="multipart/form-data">
	...
	<h2 class="entry-title"><?php _e( 'Name', 'hub' ) ?></h2>

	<?php do_action( 'bbp_user_edit_before' ); ?>

	<fieldset class="bbp-form">
		<legend><?php _e( 'Name', 'hub' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_name' ); ?>

		<div>
			<label for="first_name"><?php _e( 'First Name', 'hub' ) ?></label>
		...
		<div>
			<label for="last_name"><?php _e( 'Last Name', 'hub' ) ?></label>
		...
		<div>
			<label for="nickname"><?php _e( 'Nickname', 'hub' ); ?></label>
		...
		<div>
			<label for="display_name"><?php _e( 'Display Name', 'hub' ) ?></label>
	...
	<h2 class="entry-title"><?php _e( 'Contact Info', 'hub' ) ?></h2>

	<fieldset class="bbp-form">
		<legend><?php _e( 'Contact Info', 'hub' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_contact' ); ?>

		<div>
			<label for="url"><?php _e( 'Website', 'hub' ) ?></label>
	...
	<h2 class="entry-title"><?php bbp_is_user_home_edit() ? _e( 'About Yourself', 'hub' ) : _e( 'About the user', 'hub' ); ?></h2>

	<fieldset class="bbp-form">
		<legend><?php bbp_is_user_home_edit() ? _e( 'About Yourself', 'hub' ) : _e( 'About the user', 'hub' ); ?></legend>

		<?php do_action( 'bbp_user_edit_before_about' ); ?>

		<div>
			<label for="description"><?php _e( 'Biographical Info', 'hub' ); ?></label>
	...
	<h2 class="entry-title"><?php _e( 'Account', 'hub' ) ?></h2>

	<fieldset class="bbp-form">
		<legend><?php _e( 'Account', 'hub' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_account' ); ?>

		<div>
			<label for="user_login"><?php _e( 'Username', 'hub' ); ?></label>
		...
		<div>
			<label for="email"><?php _e( 'Email', 'hub' ); ?></label>
			...
			if ( !empty( $new_email ) && $new_email !== bbp_get_displayed_user_field( 'user_email', 'edit' ) ) : ?>
				...
				<span class="updated inline">
					...
					<?php printf( __( 'There is a pending email address change to <code>%1$s</code>. <a href="%2$s">Cancel</a>', 'hub' ), $new_email['newemail'], esc_url( self_admin_url( 'user.php?dismiss=' . bbp_get_current_user_id()  . '_new_email' ) ) ); ?>
		...
		<div id="password">
			<label for="pass1"><?php _e( 'New Password', 'hub' ); ?></label>
			<fieldset class="bbp-form password">
				...
				<span class="description"><?php _e( 'If you would like to change the password type a new one. Otherwise leave this blank.', 'hub' ); ?></span>

				<input type="password" name="pass2" id="pass2" size="16" value="" autocomplete="off" tabindex="<?php bbp_tab_index(); ?>" />
				<span class="description"><?php _e( 'Type your new password again.', 'hub' ); ?></span><br />

				<div id="pass-strength-result"></div>
				<span class="description indicator-hint"><?php _e( 'Your password should be at least ten characters long. Use upper and lower case letters, numbers, and symbols to make it even stronger.', 'hub' ); ?></span>
			</fieldset>
		</div>
	...
	<?php if ( current_user_can( 'edit_users' ) && ! bbp_is_user_home_edit() ) : ?>
		...
		<h2 class="entry-title"><?php _e( 'User Role', 'hub' ) ?></h2>

		<fieldset class="bbp-form">
			<legend><?php _e( 'User Role', 'hub' ); ?></legend>
			...
			<?php if ( is_multisite() && is_super_admin() && current_user_can( 'manage_network_options' ) ) : ?>
				...
				<div>
					<label for="super_admin"><?php _e( 'Network Role', 'hub' ); ?></label>
					<label>
						...
						<?php _e( 'Grant this user super admin privileges for the Network.', 'hub' ); ?>
					</label>
				</div>
	...
	<fieldset class="submit">
		<legend><?php _e( 'Save Changes', 'hub' ); ?></legend>
		<div>

			<?php bbp_edit_user_form_fields(); ?>

			<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_user_edit_submit" name="bbp_user_edit_submit" class="button submit user-submit"><?php bbp_is_user_home_edit() ? _e( 'Update Profile', 'hub' ) : _e( 'Update User', 'hub' ); ?></button>
		</div>
	</fieldset>
